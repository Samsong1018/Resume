...
left.append(Paragraph(
    'Incoming cybersecurity student at Seattle University and fullstack developer, after completing '
    'Running Start with an Associate of Arts and Sciences from Bellevue College. Self-taught '
    'across web development, systems administration, and security tooling — with hands-on '
    'experience deploying infrastructure, building CLI tools, and training AI models on local '
    'hardware.',
    S_BODY))
...
projects = [
    ...
    ('AMvpn — WireGuard VPN',
     'Self-hosted WireGuard VPN on Oracle Cloud with a honeypot, network-level ad blocking, '
     'and a custom dashboard unifying all of it in one log.',
     ['WireGuard', 'Oracle Cloud', 'Honeypot', 'Networking']),
    ('Homelab — Home Server & Encrypted Vault Box',
     'VPN-mesh-only, no public ports. Home server holds backups and logs with SMART alerts; '
     'a LUKS mini PC runs Vaultwarden, unlocked via early-boot SSH.',
     ['LUKS', 'Docker', 'Vaultwarden', 'Caddy', 'WireGuard']),
    ('WTFlag',
     'Claude Code hook that explains bash commands before they run, from a local SQLite '
     'tldr-pages index. No API calls.',
     ['Node.js', 'SQLite', 'CLI', 'Shell Hooks']),
    ('SENTINEL OS — Cybersecurity Training Game',
     'SOC analyst training game where every answer is computed from generated evidence. '
     'Mail triage, real Unix shell, spaced repetition. 340+ automated tests.',
     ['JavaScript', 'Node.js', 'Game Design', 'Security Education']),
    ('Somab — Voice AI Assistant (on hold)',
     'Voice assistant on a dedicated ThinkPad with custom Piper TTS voices trained on its GPU. '
     'Paused at v0.12 when the hardware became my home server.',
     ['Python', 'Piper TTS', 'Ollama', 'AI/ML']),
    ('Bumper the Band',
     'Website for my band, designed and built from scratch.',
     ['HTML', 'CSS', 'JavaScript']),
...
edu = [
    ('Cybersecurity', 'Seattle University',
     'Starting fall 2026'),
    ('Associate of Arts and Sciences', 'Bellevue College — Running Start Program',
     'Completed 2026  ·  3.75 GPA, Honor Roll  ·  Dual-enrollment through Cedarcrest High School'),
]
...
skill_groups = [
    ...
    ('Systems & Infra',  ['Linux (Ubuntu)', 'WireGuard', 'Docker', 'Oracle Cloud', 'LUKS', 'Caddy', 'Self-hosting']),
...
certs = [
    ('ISC² Certified in Cybersecurity (CC)',
     'Course Complete',
     BLUE,
     'Official training, all 5 domains complete. Exam not taken.'),
    ('Cisco — Introduction to Cybersecurity',
     'Completed',
     GREEN,
     'Threats, defenses, data protection, incident response.'),
    ('TryHackMe — Pre-Security Path',
     'Course Complete, Not Certified',
     BLUE,
